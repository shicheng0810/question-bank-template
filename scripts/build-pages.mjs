...
${cards}
  ...
  <script>
    ...
    function stripCodeFences(text){
      ...
      s = s.trim();
      // 紧跟围栏的语言标注（json/js…，无论换行还是同一行）：取开头连续字母数字，
      // 若其后跳过空白是真正的 [ 或 { 就把标注去掉（避免单行「围栏+json+数组」残留 "json " 前缀）。
      var W = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_+-";
      var m = 0;
      while (m < s.length && W.indexOf(s.charAt(m)) >= 0) m++;
      if (m > 0 && m < s.length){
        var k2 = m;
        while (k2 < s.length){ var c = s.charCodeAt(k2); if (c===32||c===9||c===10||c===13) k2++; else break; }
        var first = s.charAt(k2);
        if (first === "[" || first === "{") s = s.slice(k2);
      }
      return s.trim();
    }
